...
namespace InventoryManagement.Repository
{
    public class CommercialDataProcessing
    {
        ...
        public string FilePath = @"E:\Basic Core Program\InventoryManagement\Inventory_Management_Assignmnet\InventoryManagement\JsonFile\CustomerData.json";
        public static string filePath = @"E:\Basic Core Program\InventoryManagement\Inventory_Management_Assignmnet\InventoryManagement\JsonFile\StockData.json";
        ...
        public void Buy() 
        {
            ...
            CompanyStockAccount();
            StockAccount();
            foreach (var item in objstockData.Stock) 
            {
                if (item.StockName == stockName)
                {
                    foreach (var item2 in objstockAccounts)
                    {
                        if (item2.CustomerInfo.CustomerName == customerName)
                        {
                            ...
                            {
                                Console.WriteLine("Number of Share limit exceeded. Please buy from the given Number of Shares");
                            }
                        }
                    }
                    break;
